    ...
    <form method="post" action="<?= htmlspecialchars($basePath . '/settings/policies', ENT_QUOTES, 'UTF-8') ?>" style="display:grid; gap: 18px;">
      ...
      <label style="display:flex; gap: 12px; align-items: flex-start; font-size: 14px; cursor: pointer;">
        ...
        <span>
          ...
          <span style="color: var(--muted); font-size: 13px;">When this is on, the system may add <strong>extra ledger rows</strong> for interest that has built up but not yet been paid. Each row records interest on the previous balance (same rule as when someone makes a payment), with <strong>no payment</strong> on that row—the “due” balance becomes the new closing balance. New rows are added <strong>one calendar month</strong> after the last ledger line, until the loan reaches the number of months in the product (<code style="background: rgba(13,15,18,.06); padding: 2px 6px; border-radius: 8px;">period_months</code>) from disbursement, or until the date you pick when running accrual. Staff can run <strong>Apply accrual</strong> on an active loan (browser action uses POST only). For overnight or bulk processing, schedule <code style="background: rgba(13,15,18,.06); padding: 2px 6px; border-radius: 8px;">bin/accrue-active-loans.php</code>. Turn this <strong>off</strong> to stop automatic/monthly accrual; <strong>recording payments is unchanged</strong>.</span>
        </span>
      </label>
