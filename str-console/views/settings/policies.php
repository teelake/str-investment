    ...
    <form method="post" action="<?= htmlspecialchars($basePath . '/settings/policies', ENT_QUOTES, 'UTF-8') ?>" style="display:grid; gap: 18px;">
      ...
      <label style="display:flex; gap: 12px; align-items: flex-start; font-size: 14px; cursor: pointer;">
        ...
        <span>
          ...
          <span style="color: var(--muted); font-size: 13px;">When this is on, the system may add <strong>extra ledger rows</strong> for interest that has built up but not yet been paid. Each row records one charge on the previous balance (same rule as when someone makes a payment in a new 30-day period), with <strong>no payment</strong> on that row—the due balance becomes the new closing balance. New rows are added <strong>30 days</strong> after the last ledger line’s date, until the loan reaches the term (<code style="background: rgba(13,15,18,.06); padding: 2px 6px; border-radius: 8px;">period_months</code>) from disbursement, or until the date you pick when running accrual. Staff can run <strong>Apply accrual</strong> on an active loan (POST only). For bulk processing, schedule <code style="background: rgba(13,15,18,.06); padding: 2px 6px; border-radius: 8px;">bin/accrue-active-loans.php</code>. Turn this <strong>off</strong> to stop automatic accrual; <strong>recording payments is unchanged</strong>.</span>
        </span>
      </label>
